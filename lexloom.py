...
APP_NAME = "LexLoom"
DEFAULT_OUTPUT = "lexloom_passwords.txt"

__version__ = "3.0.0"

BANNER = r"""
  _                _                         
 | |    _____  __ | |    ___   ___  _ __ ___ 
 | |   / _ \ \/ / | |   / _ \ / _ \| '_ ` _ \
 | |__|  __/>  <  | |__| (_) | (_) | | | | | |
 |_____\___/_/\_\ |_____\___/ \___/|_| |_| |_|
...
PROFILE_KEYS = [
    'name', 'surname', 'nickname', 'partner_name', 'child_name',
    'pet', 'pet_name', 'company'
]
DATE_KEYS = ['birthday', 'partner_birthday', 'child_birthday']
GENERATION_TRIGGER_KEYS = [
    *PROFILE_KEYS, *DATE_KEYS, 'keywords', 'season', 'month',
    'weekday', 'year', 'leet', 'no_special', 'no_keyboard',
    'no_common', 'word_combo'
]
...
def parse_birthday(bd_str):
    """解析生日字符串 -> 各种日期片段"""
    digits = ''.join(ch for ch in bd_str.strip() if ch.isdigit())
    if len(digits) != 8:
        return set()
    try:
...
class LexLoomGenerator:
    ...
    def _gen_temporal(self, base_list, years):
        ...
        tails = ['']
        if self.cfg.get('special_chars', True):
            tails.extend(SPECIAL_TAILS)
...
def write_output(passwords, path):
    pw_list = unique_sorted(passwords)
...
def print_summary(count, file_size, elapsed, output_path):
    print(f"[+] 生成完成!")
    print(f"    密码总数 : {count:,}")
    print(f"    文件大小 : {size_str(file_size)}")
...
def interactive_mode():
    ...
    out = input(f"  输出文件名 (默认 {DEFAULT_OUTPUT}): ").strip() or DEFAULT_OUTPUT
    return cfg, out

def build_parser():
    p = argparse.ArgumentParser(
        description=f'{APP_NAME} - 密码字典生成器 (精简纯净版)',
...
示例:
  ...
  %(prog)s -n admin --min 8 --max 16            限制密码长度
    ...
    p.add_argument('-o', '--output', default=DEFAULT_OUTPUT, help='输出文件名')
    p.add_argument('-q', '--quiet', action='store_true', help='静默模式')
    return p

def should_generate(args):
    return any(bool(getattr(args, key, None)) for key in GENERATION_TRIGGER_KEYS)
...
def config_from_args(args):
    ...
    cfg['keywords'] = split_csv(args.keywords)
...
def main():
    ...
    if cfg.get('min_length', 1) > cfg.get('max_length', 32):
        parser.error('--min 不能大于 --max')

    if not args.quiet:
        print("\n[*] 正在生成密码字典...")
    t0 = time.time()
    
    gen = LexLoomGenerator(cfg)
    passwords = gen.run()
    ...
    if not args.quiet:
        print_summary(count, fs, elapsed, output_path)
